...
# function to show all stats on the weapon Sub page
def Show_Sub_Stats(selected_Sub, colour1, colour2):
    Show_Ability_Stats(selected_Sub, colour1, colour2)


# function to show the stats for abilities for a weapon Sub
def Show_Ability_Stats(selected_Sub, colour1, colour2):
    # load the data
    all_ability_stats_df = pd.read_csv("../data/Sub_Weapon_ability_means.csv")
    # Only get data of the chosen weapon
    ability_stats_df = all_ability_stats_df[
        all_ability_stats_df['Sub_Weapon'] == selected_Sub
        ]
    ...
    fig = px.bar(track_abilities_df,
                 ...
                 title=f"Average ability points for {selected_Sub} builds",
    ...
    fig2 = px.bar(
        ...
        title=(
            f"Usage of primary only abilities for {selected_Sub} builds"
        ),
