...
# function to show the images related to each selected main weapon
def Show_2_Ability_Image(a_img, b_img):
    # Create versions of each img with ink splat background
    A_img = Ab_img(a_img)
    B_img = Ab_img(b_img)
    # st.markdown("<br>", unsafe_allow_html=True)
    ...
    with col2:
        st.image(A_img, width=200)
    with col3:
        st.image(B_img, width=200)
...
def Ab_img(main_img):
    ...
    bg_img = "data/Small_Splats/Black.png"
    ...
    scale = 0.5  # Set the scale of the overlay
